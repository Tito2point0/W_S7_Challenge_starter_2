...
Your main goal is to implement a form that handles a pizza order, validates user input, and POSTs the order to the backend. Your next goal will be to demonstrate basic ability testing components.
...
- `fullName` length requirement ignores whitespace padding (E.G. " x " is _not_ valid despite being a string of length 3).
...
- Inside the `nav`, render two `NavLinks`:
    1. With text content **Home** that navigates to "/".
    2. With text content **Order** that navigates to "/order".
...
**Find instructions inside the [Form.js](./frontend/components/Form.js) module.**

#### 👉 TASK 5 - Write tests of your own

**Find instructions inside the [mvpB.test.js](./mvpB.test.js) module.**
...
const element = screen.getByText('hurrah')
```
